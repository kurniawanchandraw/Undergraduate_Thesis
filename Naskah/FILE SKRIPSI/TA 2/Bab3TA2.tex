...
\begin{definisi}
    ...
    \end{equation}
    dengan $\mathbf{X}_k \in \mathbb{R}^{m_k \times p}$ adalah matriks desain untuk observasi di $\mathcal{I}_k$, $\mathbf{y}_k \in \mathbb{R}^{m_k}$ adalah vektor respons untuk fold $k$ dengan $m_k = |\mathcal{I}_k|$, dan $\mathbf{W}^{(-k)}_k(\mathbf{u}_0) = \mathrm{diag}(w^{(-k)}_{i_1}, \ldots, w^{(-k)}_{i_{m_k}})$ adalah matriks bobot diagonal (\emph{cross-fitted}) untuk observasi di fold $k$ dengan parameter GNN $\widehat{\boldsymbol{\theta}}^{(-k)}$ yang dilatih tanpa data dari fold $k$.
\end{definisi}
...
\begin{teorema}
    ...
    \end{equation}
    Notasi blockwise di atas menunjukkan bahwa matriks desain $\mathbf{X}_k$, vektor respons $\mathbf{y}_k$, dan matriks bobot $\mathbf{W}^{(-k)}_k$ disusun sesuai partisi fold, dengan penjumlahan diakumulasikan lintas semua $K$ fold untuk mengintegrasikan informasi dari seluruh data sambil mempertahankan independensi struktur cross-fitting.
\end{teorema}
...
\begin{proof}
    ...
    Struktur blockwise memberikan:
    \begin{align}
        \mathbf{X}^\top \mathbf{W}_{\text{block}} \mathbf{X} &= \sum_{k=1}^K \mathbf{X}_k^\top \mathbf{W}^{(-k)}_k \mathbf{X}_k, \\
        \mathbf{X}^\top \mathbf{W}_{\text{block}} \mathbf{y} &= \sum_{k=1}^K \mathbf{X}_k^\top \mathbf{W}^{(-k)}_k \mathbf{y}_k.
    \end{align}
    Dengan demikian, persamaan normal menjadi
    \begin{equation}
        \Biggl(\sum_{k=1}^K \mathbf{X}_k^\top \mathbf{W}^{(-k)}_k \mathbf{X}_k\Biggr) \widehat{\boldsymbol{\beta}} = \sum_{k=1}^K \mathbf{X}_k^\top \mathbf{W}^{(-k)}_k \mathbf{y}_k,
    \end{equation}
    yang secara kompak dapat ditulis sebagai $(\mathbf{X}^\top \mathbf{W}_{\text{block}} \mathbf{X}) \widehat{\boldsymbol{\beta}} = \mathbf{X}^\top \mathbf{W}_{\text{block}} \mathbf{y}$. Dengan asumsi bahwa matriks $\sum_{k=1}^K \mathbf{X}_k^\top \mathbf{W}^{(-k)}_k \mathbf{X}_k$ invertibel, solusinya adalah
    \begin{equation}
...
\begin{teorema}
    ...
    dengan suku bias
    \begin{equation}
        \mathbf{B}_n(\mathbf{u}_0) = \Biggl(\sum_{k=1}^K \mathbf{H}_k\Biggr)^{-1} \Biggl(\sum_{k=1}^K \sum_{i \in \mathcal{I}_k} w^{(-k)}_i \mathbf{x}_i \mathbf{x}_i^\top \bigl(\boldsymbol{\beta}(\mathbf{u}_i) - \boldsymbol{\beta}(\mathbf{u}_0)\bigr)\Biggr)
    \end{equation}
    dan suku galat (\emph{noise term})
    \begin{equation}
        \mathbf{V}_n(\mathbf{u}_0) = \Biggl(\sum_{k=1}^K \mathbf{H}_k\Biggr)^{-1} \Biggl(\sum_{k=1}^K \sum_{i \in \mathcal{I}_k} w^{(-k)}_i \mathbf{x}_i \varepsilon_i\Biggr),
    \end{equation}
\end{teorema}
...
Selain estimasi koefisien lokal, diperlukan pula estimasi variansi galat $\sigma^2$
untuk konstruksi interval konfidensi dan uji statistik pada model GA-GWR berbasis
\emph{cross-fitting}.

\begin{teorema}
Estimator variansi galat \emph{cross-fitted} pada lokasi $\mathbf{u}_0$ diberikan oleh
\begin{equation}
\widehat{\sigma}^2_{\mathrm{CF}}(\mathbf{u}_0)
=
\frac{
\mathbf{e}^\top \mathbf{W}_{\text{block}} \mathbf{e}
}{
\operatorname{tr}(\mathbf{W}_{\text{block}}) - \operatorname{tr}\!\bigl(\mathbf{H}_{\mathrm{CF}}(\mathbf{u}_0)\bigr)
},
\end{equation}
dengan notasi blockwise $\mathbf{e} = \mathbf{y} - \mathbf{X} \widehat{\boldsymbol{\beta}}_{\mathrm{CF}}(\mathbf{u}_0)$ adalah vektor residual full (tumpukan dari $\mathbf{e}_k = \mathbf{y}_k - \mathbf{X}_k \widehat{\boldsymbol{\beta}}_{\mathrm{CF}}$ untuk setiap fold), $\mathbf{W}_{\text{block}}$ adalah matriks bobot blok diagonal terdiri dari blok-blok $\mathbf{W}^{(-k)}_k$, dan $\mathbf{H}_{\mathrm{CF}}(\mathbf{u}_0)$ adalah \emph{hat matrix} efektif pada skema \emph{cross-fitting}.
\end{teorema}

\begin{proof}
Diasumsikan bahwa galat bersyarat berdistribusi normal
$\varepsilon_i \mid \mathbf{x}_i, \mathbf{u}_i \sim \mathcal{N}(0,\sigma^2)$
dan saling bebas antar observasi.
Fungsi \emph{log-likelihood} Gaussian tertimbang lokal pada lokasi $\mathbf{u}_0$
dapat dituliskan sebagai
\begin{equation}
\ell(\boldsymbol{\beta}, \sigma^2; \mathbf{u}_0)
=
-\frac{1}{2}
...
\log(2\pi\sigma^2)
+
\frac{(y_i - \mathbf{x}_i^\top \boldsymbol{\beta})^2}{\sigma^2}
\right].
\end{equation}

Dengan mendefinisikan total bobot
\[
W_{\mathrm{tot}}(\mathbf{u}_0)
=
\sum_{k=1}^K \sum_{i \in \mathcal{I}_k} w^{(-k)}_i(\mathbf{u}_0),
\]
serta \emph{weighted residual sum of squares}
\[
\mathrm{RSS}_w(\boldsymbol{\beta})
=
\sum_{k=1}^K \sum_{i \in \mathcal{I}_k}
w^{(-k)}_i(\mathbf{u}_0)
\bigl(y_i - \mathbf{x}_i^\top \boldsymbol{\beta}\bigr)^2,
\]
fungsi \emph{log-likelihood} dapat ditulis ulang sebagai
\begin{equation}
\ell(\boldsymbol{\beta}, \sigma^2; \mathbf{u}_0)
=
-\frac{W_{\mathrm{tot}}}{2}\log(2\pi)
-
\frac{W_{\mathrm{tot}}}{2}\log(\sigma^2)
-
\frac{\mathrm{RSS}_w(\boldsymbol{\beta})}{2\sigma^2}.
\end{equation}

Turunan parsial terhadap $\sigma^2$ menghasilkan
\begin{equation}
\frac{\partial \ell}{\partial \sigma^2}
=
-\frac{W_{\mathrm{tot}}}{2\sigma^2}
+
\frac{\mathrm{RSS}_w(\boldsymbol{\beta})}{2(\sigma^2)^2}.
\end{equation}
Dengan menyamakan turunan tersebut dengan nol dan menyusun ulang, diperoleh
\begin{equation}
\widehat{\sigma}^2_{\mathrm{ML}}(\mathbf{u}_0)
=
\frac{\mathrm{RSS}_w(\widehat{\boldsymbol{\beta}}_{\mathrm{CF}})}{W_{\mathrm{tot}}}.
\end{equation}

Estimator \emph{maximum likelihood} ini bersifat bias ke bawah karena
$\widehat{\boldsymbol{\beta}}_{\mathrm{CF}}(\mathbf{u}_0)$ diperoleh melalui
estimasi parameter regresi yang mengurangi derajat kebebasan efektif.
Dalam kerangka regresi linear tertimbang, bias ini dapat dikarakterisasi oleh
\emph{hat matrix} efektif $\mathbf{H}_{\mathrm{CF}}(\mathbf{u}_0)$, sehingga
\begin{equation}
\mathbb{E}\!\left[\mathrm{RSS}_w(\widehat{\boldsymbol{\beta}}_{\mathrm{CF}})\right]
=
\sigma^2
\left(
W_{\mathrm{tot}}
-
\operatorname{tr}\!\bigl(\mathbf{H}_{\mathrm{CF}}(\mathbf{u}_0)\bigr)
\right).
\end{equation}

Dengan demikian, estimator variansi galat yang dikoreksi derajat kebebasan
diberikan oleh
\begin{equation}
\widehat{\sigma}^2_{\mathrm{CF}}(\mathbf{u}_0)
=
\frac{
\mathrm{RSS}_w(\widehat{\boldsymbol{\beta}}_{\mathrm{CF}})
}{
W_{\mathrm{tot}}
-
\operatorname{tr}\!\bigl(\mathbf{H}_{\mathrm{CF}}(\mathbf{u}_0)\bigr)
...
\end{equation}
yang bersifat tak-bias secara asimtotik.
\end{proof}

Estimator ini merupakan generalisasi alami dari estimator variansi GWR klasik
dengan koreksi derajat kebebasan.
Dalam kasus khusus satu lokasi dan satu skema pembobotan,
$\operatorname{tr}(\mathbf{H}) \approx p$ sehingga penyebut mereduksi ke bentuk
$(\sum_i w_i - p)$, analog dengan estimator variansi OLS klasik
$\widehat{\sigma}^2 = \mathrm{RSS}/(n - p)$.
...
\begin{teorema}
    \label{thm:gagwr_konsistensi_variansi_cf}
...
\begin{proof}
    ...
    Selanjutnya, dari Teorema~\ref{thm:gagwr_konsistensi_variansi} juga didapatkan
    \begin{equation}
...
\begin{proof}
    ...
    \end{equation}
    Didefinisikan $\boldsymbol{\Sigma}_R := \mathbf{R} \mathbf{Q}^{-1} \boldsymbol{\Omega} \mathbf{Q}^{-1} \mathbf{R}^\top$. Dari Teorema~\ref{thm:gagwr_konsistensi_variansi} didapatkan
    \begin{equation}
